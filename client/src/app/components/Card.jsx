...
const Card = ({ item }) => {
  return (
    <div className="flex flex-col md:flex-row gap-5 font-poppins py-4">
      <Link
        href={`/${item.id}`}
        className="flex-2 h-48 md:h-40 lg:h-48 overflow-hidden rounded-lg"
      >
        <img
          src={item.images[0]}
          alt={item.title}
      ...
      <p className="absolute  bg-purple-500 text-white px-2 py-1 text-md  rounded inline-block font-poppins font-semibold">
        {item.type}
      </p>
      <div className="flex-3 flex flex-col justify-between py-1">
        <div className="space-y-2 items-start">
          <h2 className="text-xl font-bold font-roboto dark:text-white max-w-[200px] truncate">
